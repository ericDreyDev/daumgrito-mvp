...
class ServiceRequestService {
  ...
  Future<ServiceRequest> create(ServiceRequestInput input) async {
    final data = await _apiClient.post('/service-requests', input.toJson())
  ...
  Future<ServiceRequest> getById(String id) async {
    final data =
        await _apiClient.get('/service-requests/$id') as Map<String, dynamic>;
    return ServiceRequest.fromJson(data);
  }

  Future<ServiceRequest> updateStatus(String id, String status) async {
    final data =
        await _apiClient.put('/service-requests/$id/status', {'status': status})
            as Map<String, dynamic>;
    return ServiceRequest.fromJson(data);
  }
}
